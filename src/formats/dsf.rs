...
//! `'fmt '` chunk. Read-only.
//!
//! The ID3v2 trailer arm (DSF.pm:88-97) is FAITHFULLY DEFERRED to the
//! parallel ID3 PR (FORMATS.md row 2). When ID3 ships, that PR re-lands the
//! `if ($metaPos and $metaLen > 0 and $metaLen < 20_000_000 and seek+read)
//! { ProcessDirectory(ID3::Main) }` arm here.
...
impl FormatParser for ProcessDsf {
  fn process(&self, ctx: &mut ParseContext<'_>) -> bool {
    ...
    let _meta_pos: u64;
    {
      ...
      // \$buff,20)` — local-only, NOT emitted as DSF tags. Read here so the
      // ID3 trailer port (DSF.pm:88-97) has them ready when it lands;
      // suppressed with `_` until then.
      let mut le = [0u8; 8];
      le.copy_from_slice(&head[12..20]);
      _file_size = u64::from_le_bytes(le);
      let mut le = [0u8; 8];
      le.copy_from_slice(&head[20..28]);
      _meta_pos = u64::from_le_bytes(le);
    }
    ...
    // DSF.pm:88-97 ID3v2 trailer: when `$metaPos and $metaLen > 0 and
    // $metaLen < 20_000_000` AND `$raf->Seek($metaPos,0) and $raf->Read(
    // $buff, $metaLen) == $metaLen`, run `ProcessDirectory(ID3::Main)` over
    // `$buff` at file position `$metaPos`. FAITHFULLY DEFERRED to the
    // parallel ID3 PR (FORMATS.md row 2) — when ID3 ports, that PR re-lands
    // the arm here. Until then DSF files with an ID3 trailer parse the fmt
    // chunk tags successfully and the ID3 trailer is silently ignored
    // (faithful behavior because DSF.pm itself ignores the trailer when ID3
    // isn't loaded, except DSF.pm always loads ID3 — exifast is not yet
    // there). No fixture in this PR exercises a non-zero metaPos.
