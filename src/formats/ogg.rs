...
static VC_ARTIST: TagDef = TagDef::new("Artist", "Vorbis", ValueConv::None, PrintConv::None);
// Vorbis.pm:86 PERFORMER => List=>1
static VC_PERFORMER: TagDef = TagDef::new("Performer", "Vorbis", ValueConv::None, PrintConv::None);
...
static VC_CONTACT: TagDef = TagDef::new("Contact", "Vorbis", ValueConv::None, PrintConv::None);
...
/// Vorbis.pm:85, 86, 94: keys with `List => 1`. Repeated occurrences are
/// accumulated into a `TagValue::List` (rather than dropped by `%noDups`).
fn vorbis_comment_is_list(key: &str) -> bool {
  matches!(key, "ARTIST" | "PERFORMER" | "CONTACT")
}
...
fn process_vorbis_comments(data: &[u8], meta: &mut Metadata, print_conv_enabled: bool) -> bool {
  ...
  // Accumulator for `List => 1` tags (ARTIST, PERFORMER, CONTACT). The
  // per-key `Vec<TagValue>` preserves insertion order WITHIN one tag (so a
  // multi-valued ARTIST keeps its source order). Across-tag emission order
  // is deterministic but NOT insertion order: `finalize_list_accumulators`
  // sorts keys lexicographically before flushing (the only across-tag
  // sequence ExifTool observes is the alphabetical names ARTIST, CONTACT,
  // PERFORMER — i.e. the sorted order — so the two coincide in practice).
  let mut list_accumulators: HashMap<String, Vec<TagValue>> = HashMap::new();
  // Read `num` comments.
  for _ in 0..num {
    let Some(comment_len) = read_u32_le(data, pos) else {
      meta.push_warning("Format error in Vorbis comments");
      finalize_list_accumulators(&mut list_accumulators, meta, print_conv_enabled);
      return false;
    };
    ...
    push_vorbis_comment_with_accumulator(
      meta,
      &key,
      TagValue::Str(SmolStr::from(raw_val)),
      print_conv_enabled,
      Some(&mut list_accumulators),
    );
  }
  finalize_list_accumulators(&mut list_accumulators, meta, print_conv_enabled);
...
/// Push a single non-list Vorbis comment. For the `vendor` key, the
/// resolved name is "Vendor" via [`vorbis_comment_known`]; for unknown
/// keys, the name is computed via [`vorbis_comment_compute_name`].
fn push_vorbis_comment(
  meta: &mut Metadata,
  key: &str,
  raw: TagValue,
  print_conv_enabled: bool,
  list_accumulators: Option<&mut HashMap<String, Vec<TagValue>>>,
) {
  if vorbis_comment_is_list(key) {
    // List handled separately by the caller; this fn is the non-list path.
    // Caller should always call the with-accumulator variant for these.
    // Defensive: if invoked anyway, accumulate.
    if let Some(acc) = list_accumulators {
      acc.entry(key.to_string()).or_default().push(raw);
      return;
    }
  }
  if let Some(def) = vorbis_comment_known(key) {
    let out = apply(def, &raw, print_conv_enabled);
    meta.push(Group::new("Vorbis", def.group1()), def.name(), out);
  } else {
...
/// Like [`push_vorbis_comment`] but routes list-tag occurrences into
/// the per-key accumulator (caller flushes at end of parse).
fn push_vorbis_comment_with_accumulator(
  meta: &mut Metadata,
  key: &str,
  raw: TagValue,
  print_conv_enabled: bool,
  ...
  push_vorbis_comment(meta, key, raw, print_conv_enabled, None);
}

/// Drain the list-tag accumulators into the metadata as `TagValue::List`
/// (or single scalar if only one element, matching ExifTool's behaviour:
/// a single ARTIST entry stays a scalar, multiple become a list).
fn finalize_list_accumulators(
  list_accumulators: &mut HashMap<String, Vec<TagValue>>,
  meta: &mut Metadata,
  print_conv_enabled: bool,
) {
  // Determinism: `HashMap` iteration is unordered, so we sort keys
  // lexicographically before flushing. ExifTool's three List-typed Vorbis
  // comment names are ARTIST, CONTACT, PERFORMER — already sorted — so this
  // alphabetical flush order coincides with the natural emission order
  // bundled-Perl exiftool produces (the conformance test against
  // `multi_artist.ogg` would catch a deviation). Within one key, the
  // accumulator's `Vec<TagValue>` preserves source occurrence order.
  let mut keys: Vec<String> = list_accumulators.keys().cloned().collect();
  keys.sort();
  for key in keys {
    let values = list_accumulators.remove(&key).unwrap_or_default();
    if values.is_empty() {
      continue;
    }
    ...
    if let Some(def) = vorbis_comment_known(&key) {
      let out = apply(def, &value, print_conv_enabled);
      meta.push(Group::new("Vorbis", def.group1()), def.name(), out);
    } else {
      // Unknown List keys: dynamic name, no conv.
      let name = vorbis_comment_compute_name(&key);
      meta.push(Group::new("Vorbis", "Vorbis"), SmolStr::from(name), value);
    }
  }
}
