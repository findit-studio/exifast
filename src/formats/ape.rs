...
//! - Embedded ID3v1/v2 scan (APE.pm:124-127): Phase-2 row 2 (ID3) dependency.
//!   Until ID3 ships, an APE file with leading ID3v2 or trailing ID3v1 may
//!   be (a) detected as ID3/MP3 by the engine's filetype magic before
//!   reaching ProcessApe (src/filetype_data.rs:861-865 includes the `ID3`
//!   prefix arm of the APE magic), or (b) mis-extract here.
//! - specialTags `.` suffix collision (APE.pm:209): no real collision exists
//!   for the APE Main table.
//! - `footPos -= $$et{DoneID3}` (APE.pm:169): pure ID3-dependent shift of
//!   the APETAGEX footer search; defers until ID3.
...
impl FormatParser for ProcessApe {
  fn process(&self, ctx: &mut ParseContext<'_>) -> bool {
    // === APE.pm:124-127 EMBEDDED ID3 — DEFERRED ============================
    // `unless ($$et{DoneID3}) { require Image::ExifTool::ID3;
    //  ID3::ProcessID3(...) and return 1; }`
    //
    // Needs ID3.pm (FORMATS.md row 2). The engine's APE %magicNumber
    // (src/filetype_data.rs:861-865) faithfully includes the `ID3` prefix
    // arm (mirroring ExifTool.pm:933 `'(MAC |APETAGEX|ID3)'`), so a
    // `.ape` file starting with `ID3` would be dispatched here. Our
    // `plan_ape` only accepts `MAC ` or `APETAGEX` and rejects everything
    // else with `false` — that lets `extract_info` fall through to the
    // next candidate (today: another ID3-magic-claiming format like MP3,
    // also unported ⇒ "File format error"). Codex r1 finding 2: this
    // diverges from bundled ExifTool for ID3-prefixed APE files. Known
    // and tracked. The faithful fix lands with ID3.pm (row 2) — at that
    // point, this ID3 branch will dispatch to `ID3::ProcessID3` and
    // `return 1` if it accepts, exactly per APE.pm:125-126.

    let print_conv_enabled = ctx.print_conv_enabled();

    // ----- Phase 1: planning (read-only over ctx.data()) -------------------
    let plan = match plan_ape(ctx.data(), print_conv_enabled) {
      Some(p) => p,
      None => return false, // APE.pm:137-138 short or wrong-magic ⇒ return 0.
    };
    ...
    // ----- Phase 2: header + tag-stream emission ---------------------------
...
impl ProcessApe {
  ...
  /// `ID3::ProcessID3` then `APE::ProcessAPE`, or an `MPC`/`WavPack`
  /// parser then APE-trailer-tags). Skips the magic check + SetFileType
  /// and binary-header block (APE.pm:137-162) and runs ONLY the
  /// APETAGEX-trailer scan (APE.pm:165-237).
  ///
  /// Today's engine dispatches one parser per file, so this is currently
  /// unreachable from production. The method is `pub(crate)` so it can
  /// be wired by future format ports as their chained-parser follow-on
  /// for APE-tag-trailer extraction.
  #[allow(dead_code)] // Phase-1: no chained-parser consumer in-tree yet; Phase-2 will wire it.
  pub(crate) fn process_trailer_only(&self, ctx: &mut ParseContext<'_>) -> bool {
    let print_conv_enabled = ctx.print_conv_enabled();
    let plan = match plan_ape_trailer_only(ctx.data(), print_conv_enabled) {
      Some(p) => p,
      None => return false, // unreachable (trailer-only always returns Some)
    };
    // header_job is HeaderJob::None for this path — no SetFileType, no
    // MAC header. The caller (chained parser) owns those.
    let warn = plan.warn_bad_trailer;
    let pending = plan.pending;
    ctx.metadata_then(|m| {
      for (g1, name, value) in pending {
...
fn plan_ape(data: &[u8], print_conv_enabled: bool) -> Option<ApePlan> {
  plan_ape_inner(data, print_conv_enabled, false)
}
...
/// MPC, WavPack chains).
///
/// Today's engine dispatches one parser per file, so the in-tree driver
/// [`ProcessApe::process`] does NOT call this. It is `pub(crate)` (Codex
/// r15 finding) so future format ports — MPC.pm (FORMATS.md row 4),
/// WavPack.pm (row 6), ID3-then-MP3 (row 2) — can dispatch into the
/// APE trailer extraction directly via
/// [`ProcessApe::process_trailer_only`]. Unit-tested via the same
/// seam; pinned for the chained-parser use case.
#[allow(dead_code)] // Phase-1: no chained-parser consumer in-tree yet; Phase-2 will wire it.
pub(crate) fn plan_ape_trailer_only(data: &[u8], print_conv_enabled: bool) -> Option<ApePlan> {
  plan_ape_inner(data, print_conv_enabled, true)
}
...
fn plan_apetagex_trailer_only(data: &[u8], print_conv_enabled: bool) -> ApePlan {
  let mut plan = ApePlan {
    header_job: HeaderJob::None,
    pending: Vec::new(),
    warn_bad_trailer: false,
  };
  // APE.pm:166-170 `$footPos = -32; $raf->Seek($footPos, 2) or return 1;
  // $raf->Read($buff, 32) == 32 or return 1;` — need ≥ 32 bytes to even
  // attempt the trailer read.
  if data.len() < 32 {
    return plan;
  }
  let foot_off = data.len() - 32;
  let footer = &data[foot_off..];
  ...
  match decode_apetagex_body_size(size_raw) {
    ...
    Some(body_size) => match foot_off.checked_sub(body_size) {
      None => plan.warn_bad_trailer = true, // seek-back would underflow
      Some(body_start) => {
        let body = &data[body_start..foot_off];
...
fn plan_ape_inner(data: &[u8], print_conv_enabled: bool, already_typed: bool) -> Option<ApePlan> {
  ...
  if already_typed {
    return Some(plan_apetagex_trailer_only(data, print_conv_enabled));
  }
  ...
  let header_state: HeaderState<'_> = if header_at_start {
    ...
    // APE.pm:165-174 footer path (data.len() >= 32 enforced above).
    let foot_off = data.len() - 32;
    let footer = &data[foot_off..];
    if !footer.starts_with(b"APETAGEX") {
      ...
      match decode_apetagex_body_size(size_raw) {
        ...
        Some(body_size) => {
          ...
          // seek succeeds iff `foot_off >= body_size`. The subsequent
          // `Read($buff, $size) == $size` then succeeds (the bytes exist
          // between body_start and foot_off, which is `body_size` bytes).
          match foot_off.checked_sub(body_size) {
            Some(body_start) => HeaderState::HeaderAndBody(footer, &data[body_start..foot_off]),
            None => HeaderState::HeaderInvalid,
          }
        }
      }
...
mod tests {
  ...
  fn trailer_only_plan_extracts_apetagex_without_magic_header() {
    ...
    let plan = plan_ape_trailer_only(&data, true).expect("trailer-only plan returns Some");
