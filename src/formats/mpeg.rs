...
impl FormatParser for ProcessMp3 {
  fn process(&self, ctx: &mut ParseContext<'_>) -> bool {
    // ID3.pm:1684-1729 `sub ProcessMP3`: when not ID3-tagged (which we
    // aren't yet — the ID3 parallel PR will wrap THIS parser), the
    // recover path is `$raf->Read($buff, $scanLen)` + `ParseMPEGAudio($et,
    // \$buff, $mp3)`. The Perl `$buff` is a HARD slice — the bytes
    // BEYOND `$scanLen` are simply absent from the parser's view, so a
    // late sync byte cannot be latched. We emulate by bounding `data` to
    // `scan_len` BEFORE invoking `parse_mpeg_audio`; `set_file_type`'s
    // first-call-wins guard makes the bounded buffer behave identically
    // for the SetFileType branch.
    //
    ...
    let (data, meta) = ctx.data_and_metadata();
    let bounded_len = scan_len.min(data.len());
    let bounded = &data[..bounded_len];
