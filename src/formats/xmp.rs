...
// Golden-v2 Contract 3c: parser-panic-safety by construction. Every raw
// `buf[i]` / `buf[a..b]` on input bytes is a checked `.get()` (byte-identical
// to the unchecked form on the happy path, but `None` instead of a panic on a
// malformed/truncated XMP). The `#[cfg(test)]` module opts out locally.
...
impl FormatParser for ProcessXmp {
  ...
  /// Run `ProcessXMP` (XMP.pm:4262) on the raw sidecar bytes. Returns `None`
  /// for any input bundled rejects with `return 0` (a non-XMP root, an
  /// `<svg`-rooted document, …); a malformed-but-recognized XMP is accepted
  /// with a recorded warning surfaced through [`Diagnose`](crate::diagnostics::Diagnose).
  fn parse<'a>(&self, data: Self::Context<'a>) -> Option<Self::Meta<'a>> {
    parse_inner(data)
  }
}

/// Lib-first direct entry — faithful port of
/// `Image::ExifTool::XMP::ProcessXMP` (XMP.pm:4262). Returns `None` for any
/// input `ProcessXMP` rejects with `return 0`; a malformed-but-recognized XMP
/// is accepted with a recorded warning (surfaced via the
...
// Bounds-guarded byte/str walker: every `[i]` / `[a..b]` here is proven
// in-range by an explicit `i < end` / `i + N <= len` check (with `&&`
// short-circuit) or a fixed-size array, and the walker's `end`-window
// bound is load-bearing — a `.get()` rewrite would change the bound from
...
/// warning set).
// Bounds-guarded byte walker: every `bytes[i]` is proven in-range by the
// enclosing `while i < bytes.len()` (with `&&` short-circuit on the inner
// continuation scan). The file-level `#![deny]` still catches any NEW
// unchecked indexing added outside these proven helpers.
#[allow(clippy::indexing_slicing)]
...
#[derive(Default)]
struct BlankNode {
  /// `$$blankInfo{Prop}{$id}{Pre}` — the distinct subject/object path
  /// PREFIXES seen for this node (the `$pre` strings, already routed per the
  /// subject/object + `rdf:Description` rule of `SaveBlankInfo`).
  pre: BTreeMap<String, ()>,
  /// `$$blankInfo{Prop}{$id}{Post}` — keyed by the `$post` suffix STRING
  /// (e.g. `"/exif:Fired"`, or `""` for a bare subject), value
  /// `[$val, $propPath]` — the property value and the FULL original property
  /// path (used by the `%unused` malformed-RDF fallback, WriteXMP.pl:497).
  post: BTreeMap<String, (String, String)>,
}
...
impl Walker<'_> {
  ...
  fn parse_element(
    ...
    loop {
      ...
      let Some(tok) = scan_token(bytes, lt, end) else {
        // The token regex (XMP.pm:3808
        // `<([?/]?)([-\w:.\x80-\xff]+|!--)([^>]*)>|(<!\[CDATA\[)`) failed at
        // this `<`. Distinguish the Perl outcomes:
        //  * an UNTERMINATED top-level CDATA — XMP.pm:3812-3816 sets
        //    `$err = "Missing CDATA terminator"; last Element`, so this WARNS;
        //  * an UNTERMINATED top-level comment — XMP.pm:3819-3822 does a bare
        //    `last` with NO `$err`, so this STOPS the level WITHOUT a warning
        //    (the documented asymmetry — only top-level CDATA warns);
        //  * any OTHER non-match (a stray `<` in text like `bad < text`, a
        //    `<` not followed by a name char, or a name with no closing `>`)
        //    — the `/g` engine simply advances ONE byte and keeps scanning.
        //    Skipping such a `<` through its (possibly later) `>` would
        //    swallow a real following element, dropping that tag AND every
        //    later sibling.
        let after = lt + 1;
        let is_cdata_opener = bytes
          .get(after..)
          .is_some_and(|s| s.starts_with(b"![CDATA["));
        let is_comment_opener = bytes.get(after..).is_some_and(|s| s.starts_with(b"!--"));
        if is_cdata_opener && after < end {
          // Unterminated top-level CDATA — `$err`; `last Element` (warns).
          self.warn("Missing CDATA terminator".to_string());
          break;
        }
        if is_comment_opener && after < end {
          // Unterminated top-level comment — bare `last` (no warning).
          break;
        }
        pos = lt + 1;
        continue;
      ...
      match tok {
        ...
        Token::Element {
          ...
          let mut element = if empty {
            ...
            let (val_end, next, was_comment) =
              match find_close(bytes, &prop_raw, content_start, end) {
                Ok(t) => t,
                Err(e) => {
                  // Each close-scan failure sets the matching `$err`
                  // (XMP.pm:3839/3845/3849), emitted once as `$et->Warn($err)`
                  // on the read path (XMP.pm:4237) before `last Element`.
                  self.warn(match e {
                    CloseErr::NoClosingTag => {
                      std::format!("XMP format error (no closing tag for {prop_raw})")
                    }
                    CloseErr::MissingCdata => "Missing CDATA terminator".to_string(),
                    CloseErr::MissingComment => "Missing comment terminator".to_string(),
                  });
                  break;
                }
              };
          ...
          let outcome = self.handle_element(
            &mut element,
            prop_list,
            node_id,
            &mut n_items,
          ...
          // A suppressed `et:desc`/`et:prt`/`et:val` qualifier (XMP.pm:4202)
          // does `--$count` then `++$count` — net no-op — so it must NOT
          // advance the sibling count (the next sibling sees the same
          // `$count` it would have without the qualifier).
          if outcome != ElementOutcome::ProcessedNoCount {
            count += 1;
          }
          pos = element.next;
  ...
  fn handle_element(
    ...
    node_id: Option<&str>,
    n_items: &mut usize,
    // The level scanner's `$count` (XMP.pm:3772) — the number of SIBLING
    // elements already fully processed at this nesting level. Read by the
    // `et:desc`/`et:prt`/`et:val` suppression branch (XMP.pm:4202): `et:val`
    // and `et:prt` are ignored only when `$count` is truthy (not the first
    // sibling); `et:desc` is always ignored.
    count: usize,
    ...
    // The element's outcome — `Processed` unless an `et:desc`/`prt`/`val`
    // qualifier is suppressed below (then `ProcessedNoCount`, matching Perl's
    // `--$count`; XMP.pm:4202-4204).
    let mut outcome = ElementOutcome::Processed;
    if !descended {
      ...
      if !val.is_empty() || !shorthand {
        ...
        } else if let Some(kind) = et_qual_suffix(last_prop)
          && (count != 0 || kind == EtQual::Desc)
        {
          // `elsif ($lastProp =~ /^et:(desc|prt|val)$/ and ($count or
          // $1 eq 'desc'))` (XMP.pm:4202): IGNORE `et:desc` always, and
          // `et:prt`/`et:val` when NOT the first sibling (`$count` truthy).
          // ExifTool then `--$count` (XMP.pm:4204), undoing the unconditional
          // `++$count`; we signal that via `ProcessedNoCount` so the level
          // scanner skips its `count += 1`. The value is NOT extracted —
          // so for `<foo:Tag><et:desc>D</et:desc><et:prt>Print</et:prt>
          // <et:val>Value</et:val></foo:Tag>`, `et:prt` (the first sibling,
          // `$count==0`) IS extracted as the value and `et:desc`/`et:val`
          // are dropped (bundled: `XMP-foo:Tag=Print`). `GetXMPTagID` already
          // strips the `et:*` suffix (`%ignoreEtProp`), so the surviving
          // qualifier records under the PARENT property name.
          outcome = ElementOutcome::ProcessedNoCount;
        } else {
...
enum ElementOutcome {
  ...
  Processed,
  /// An `et:desc`/`et:prt`/`et:val` qualifier was SUPPRESSED (XMP.pm:4202):
  /// the value was not extracted AND the sibling count must NOT advance for
  /// it — Perl does `--$count` in the branch (XMP.pm:4204) then the
  /// unconditional `++$count` (XMP.pm:4213), a net no-op, so this variant
  /// tells the level scanner to skip its `count += 1`. Scanning continues.
  ProcessedNoCount,
...
/// Why [`find_close`] failed to locate an element's close token — the three
/// `$err` outcomes of the XMP.pm:3836-3861 nesting loop, mapped 1:1 so the
/// caller raises the matching `$et->Warn($err)` (XMP.pm:4237) on the read path.
enum CloseErr {
  /// No matching close token before end-of-data — XMP.pm:3839
  /// `"XMP format error (no closing tag for $prop)"`. Covers the regex
  /// failing to match (no more `<`, or `pos > $end`) and a same-name token
  /// that never finds its own `>`.
  NoClosingTag,
  /// An in-content `<![CDATA[` with no `]]>` — XMP.pm:3845
  /// `"Missing CDATA terminator"`.
  MissingCdata,
  /// An in-content `<!--` with no `-->` — XMP.pm:3849
  /// `"Missing comment terminator"`.
  MissingComment,
}

/// Find the close tag of a non-empty element, honoring same-name nesting.
/// Faithful to the XMP.pm:3836-3861 nesting loop, whose close-scan is the
/// `/sg` regex
/// `<(?:(/?)\Q$prop\E([-\w:.\x80-\xff]*)(.*?(/?))>|(!\[CDATA\[|!--))`:
///
/// * The match is UNANCHORED — a `<` that does NOT begin
///   `[/]?\Q$prop\E…>` (a stray `<` in text content like `bad < text`, or a
///   different-named element `<dc:format>`) is simply NOT matched, so the
///   `/g` engine advances ONE byte and keeps scanning for the real
///   `</prop>` / a nested same-name `<prop…>`. It must NOT be consumed
///   through its own `>` (doing so would swallow a following real close).
/// * `$2` (`[-\w:.\x80-\xff]*`) captures any NAME-EXTENSION chars after the
///   exact prop name; `next if $2` (XMP.pm:3853) SKIPS a token whose name
///   merely STARTS with `prop` — e.g. for `prop = dc:title`, a
///   `</dc:titleExtra>` has `$2 = "Extra"` and is IGNORED (no nesting
///   change), not treated as the close.
/// * `$1` = leading `/` ⇒ a close token: `--$nesting`; at zero this element
///   is complete. `$4` = a trailing `/` before `>` ⇒ an empty element, so
///   nesting is NOT incremented for it.
/// * `$5` = a CDATA / comment opener: skip to `]]>` / `-->`.
///
/// On success returns `Ok((content_end, next_pos, was_comment))`. On failure
/// it returns the specific [`CloseErr`] so the caller can raise the matching
/// `$err` `Warn` (XMP.pm:3839/3845/3849; emitted once at XMP.pm:4237) before
/// `last Element`.
// Bounds-guarded byte/str walker: every `[i]` / `[a..b]` here is proven
// in-range by an explicit `i < end` / `i + N <= len` check (with `&&`
// short-circuit) or a fixed-size array, and the walker's `end`-window
...
) -> Result<(usize, usize, bool), CloseErr> {
  let prop_bytes = prop.as_bytes();
  let mut pos = content_start;
  let mut nesting = 1i32;
  let mut was_comment = false;
  loop {
    // Find the next `<` that begins a relevant token. No more `<` ⇒ the regex
    // never matches a close — XMP.pm:3839 `last Element` with the no-closing
    // `$err`.
    let Some(rel) = bytes[pos..end].iter().position(|&b| b == b'<') else {
      return Err(CloseErr::NoClosingTag);
    };
    let lt = pos + rel;
    let after = lt + 1;
    if after >= end {
      // `pos > $end` after a bare trailing `<` — XMP.pm:3837.
      return Err(CloseErr::NoClosingTag);
    }
    // `$5` — CDATA / comment opener inside content: skip to terminator.
    if bytes[after..end].starts_with(b"![CDATA[") {
      let body = after + 8;
      // No `]]>` within the window — XMP.pm:3845 `"Missing CDATA terminator"`.
      let Some(close) = find_sub(&bytes[body..end], b"]]>") else {
        return Err(CloseErr::MissingCdata);
      };
      pos = body + close + 3;
      continue;
    }
    if bytes[after..end].starts_with(b"!--") {
      let body = after + 3;
      // No `-->` within the window — XMP.pm:3849 `"Missing comment terminator"`.
      let Some(close) = find_sub(&bytes[body..end], b"-->") else {
        return Err(CloseErr::MissingComment);
      };
      pos = body + close + 3;
      was_comment = true;
      continue;
    }
    // Does this `<` begin `[/]?\Q$prop\E`? `(/?)` is the optional close
    // slash (`$1`); the prop name must then match EXACTLY.
    let is_close = bytes[after] == b'/';
    let name_at = if is_close { after + 1 } else { after };
    if !bytes[name_at..end].starts_with(prop_bytes) {
      // Not our element (a stray `<`, or a different-named element). The
      // Perl regex does not match here, so `/g` advances one byte — do NOT
      // skip through this token's `>` (that would swallow a later real
      // close, dropping the tag AND its siblings).
      pos = lt + 1;
      continue;
    }
    // The byte right after the exact prop name decides `$2`: if it is a
    // name-extension char, `$2` is non-empty (the name merely STARTS with
    // `prop`, e.g. `dc:titleExtra`); `next if $2` skips the whole token.
    let after_name = name_at + prop_bytes.len();
    // Locate the token's `>` (`(.*?(/?))>`; `.*?` is non-greedy under `/s`,
    // so this is the FIRST `>` after the name). A same-name token with no `>`
    // means the regex `(.*?(/?))>` cannot complete — XMP.pm:3839 no-closing.
    let Some(gt_rel) = bytes[after_name..end].iter().position(|&b| b == b'>') else {
      return Err(CloseErr::NoClosingTag);
    };
    let gt = after_name + gt_rel;
    let token_end = gt + 1;
    if after_name < end && is_name_char(bytes[after_name]) {
      // `$2` non-empty — different name: skip this token, nesting unchanged.
      pos = token_end;
      continue;
    }
    if is_close {
      // `$1` set — a close token for this exact prop.
      nesting -= 1;
      if nesting == 0 {
        // `$valEnd = pos - length($prop) - length($3) - 3` (XMP.pm:3856) ==
        // the byte offset of this close token's `<`.
        return Ok((lt, token_end, was_comment));
      }
      pos = token_end;
      continue;
    }
    // An opening token for this exact prop. `$4` — a trailing `/` before
    // `>` — marks an empty element, which does NOT nest (`++$nesting
    // unless $4`).
    let is_empty = gt > after_name && bytes[gt - 1] == b'/';
    if !is_empty {
      nesting += 1;
    }
    pos = token_end;
...
impl Walker<'_> {
  ...
  fn register_namespace(
    ...
    if std_ns.is_none() {
      // Patch for a trailing-slash URI bug (XMP.pm:3909-3913). XMP.pm:3911
      // `$try =~ s{/$}{} or $try .= '/'` toggles EXACTLY ONE trailing slash:
      // `s{/$}{}` strips a single final `/` (so `.../1.0//` → `.../1.0/`, which
      // still matches the known URI), NOT every trailing slash. `strip_suffix`
      // removes one occurrence; `trim_end_matches` would wrongly remove all and
      // misclassify a repairable camera namespace (e.g. `exif/1.0//`).
      let try_uri = if let Some(stripped) = val.strip_suffix('/') {
        stripped.to_string()
      } else {
        std::format!("{val}/")
      };
      if let Some(s) = uri_to_ns(&try_uri) {
        // XMP.pm:3914-3915 — the trailing-slash patch matched a known URI:
        // `$val = $try; $et->Warn("Fixed incorrect URI for xmlns:$ns", 1)`.
        // The `, 1` marks it MINOR; the single-string XMP warning channel
        // carries no per-warning `ignorable` level, so (as with the `[Minor]`
        // `rdf:li`-cap warning) the `[minor] ` prefix is embedded literally —
        // `diagnostics()` yields it via `Diagnostic::warn` (ignorable 0), and
        // `apply_minor_prefix` adds nothing further, so the surfaced string is
        // exactly the bundled `[minor] Fixed incorrect URI for xmlns:<ns>`.
        self.warn(std::format!("[minor] Fixed incorrect URI for xmlns:{ns}"));
        *val = try_uri;
...
/// run. Faithful to the COMMON (`length < 2000`) branch of the XMP.pm
/// `ParseXMPElement` attribute loop (XMP.pm:3884-3900):
///
/// ```text
/// for (;;) {
///     last unless $attrs =~ /(\S+?)\s*=\s*(['"])/g;   # name = quote
///     ($attr, $quote) = ($1, $2);
///     my $p0 = pos($attrs);
///     last unless $attrs =~ /$quote/g;                # value up to same quote
///     my $val = substr($attrs, $p0, pos($attrs)-$p0-1);
/// }
/// ```
///
/// The first regex is UNANCHORED with `/g`: at each iteration the engine finds
/// the NEXT substring matching `\S+?\s*=\s*['"]` from the current `pos`. A junk
/// token with no following `=quote` is therefore SKIPPED, not fatal — e.g.
/// `rdf:about="" xmlns:dc="…" junk dc:title="Lost"` still yields the `dc:title`
/// attribute. We mirror that: on a malformed candidate we ADVANCE one byte and
/// keep scanning for the next `name\s*=\s*quote` pair rather than terminating.
///
/// `\S+?` is a NON-GREEDY run of non-space bytes (it may contain `=` and quote
/// bytes — the engine only stops a name early because a `\s*=\s*['"]` match
/// follows); a name is therefore the shortest non-space prefix, anchored at a
/// non-space byte, such that optional ws, `=`, optional ws, and a quote follow.
///
/// The `>= 2000`-byte garbage-avoidance branch (XMP.pm:3890-3896, a 13.23
/// anti-hang patch using `/=\s*(['"])/g` + a 1000-byte backscan) is treated as
/// an accepted anti-DoS deferral; only the common branch is implemented here
/// (real sidecar attribute runs are far under 2000 bytes).
// Bounds-guarded byte/str walker: every `[i]` / `[a..b]` here is proven
// in-range by an explicit `i < len` / `i + N <= len` check (with `&&`
// short-circuit). The file-level `#![deny]` still catches any NEW unchecked
// indexing added outside this proven helper.
#[allow(clippy::indexing_slicing)]
fn iter_attrs(text: &str) -> Vec<(String, String)> {
  let bytes = text.as_bytes();
  let len = bytes.len();
  let mut out = Vec::new();
  // `i` is Perl's `pos($attrs)` — the offset the next `/g` search resumes from.
  let mut i = 0;
  while i < len {
    // Find the next `\S+?\s*=\s*['"]` match at-or-after `i` (the unanchored
    // `/g` search). `name_start..name_end` is the captured `\S+?` name and
    // `quote_pos` indexes the opening quote byte.
    let Some((name_start, name_end, quote_pos)) = next_attr_match(bytes, i) else {
      break; // no further `name = quote` — `last unless …` in Perl
    };
    let quote = bytes[quote_pos];
    // Value runs from just past the opening quote to the next same quote
    // (`last unless $attrs =~ /$quote/g`). An unterminated value ends the run.
    let val_start = quote_pos + 1;
    let mut j = val_start;
    while j < len && bytes[j] != quote {
      j += 1;
    }
    if j >= len {
      break; // no closing quote — `last unless …`
    }
    out.push((
      text[name_start..name_end].to_string(),
      text[val_start..j].to_string(),
    ));
    i = j + 1; // resume past the value's closing quote
  }
  out
}

/// Locate the next `\S+?\s*=\s*['"]` match at-or-after `from`, mirroring the
/// leftmost / non-greedy semantics of Perl's `/(\S+?)\s*=\s*(['"])/g`. Returns
/// `(name_start, name_end, quote_pos)` for the captured name run and the
/// opening-quote byte, or `None` if no match remains.
// Bounds-guarded byte walker (see `iter_attrs`): every `[i]` is `i < len`
// guarded; no slicing.
#[allow(clippy::indexing_slicing)]
fn next_attr_match(bytes: &[u8], from: usize) -> Option<(usize, usize, usize)> {
  let len = bytes.len();
  // Leftmost match: try each candidate start `s` from `from` upward. `\S+?`
  // must begin on a non-space byte, so whitespace bytes can never start a
  // match and are skipped implicitly.
  let mut s = from;
  while s < len {
    if bytes[s].is_ascii_whitespace() {
      s += 1;
      continue;
    }
    // `s` is a non-space byte: the start of a candidate name. `\S+?` is
    // non-greedy, so try the SHORTEST name first — advance the name end `n`
    // through the contiguous non-space run and, at each split, test whether
    // `\s*=\s*['"]` follows. The first (shortest) `n` that matches wins for
    // this start `s`, and `s` being the earliest non-space start makes it the
    // overall leftmost match.
    let mut n = s + 1; // name is `\S+?` → at least one byte
    while n <= len {
      // The name run `s..n` must be all non-space (`\S+?`); stop extending the
      // candidate once a whitespace byte is reached.
      if n > s + 1 && bytes[n - 1].is_ascii_whitespace() {
        break;
      }
      if let Some(quote_pos) = match_eq_quote(bytes, n) {
        return Some((s, n, quote_pos));
      }
      n += 1;
    }
    // No split of the run at `s` matched; advance one byte and retry (this is
    // where Perl's engine skips junk tokens like `junk` before `dc:title=`).
    s += 1;
  }
  None
}

/// Match `\s*=\s*['"]` anchored at byte offset `at`, returning the index of the
/// opening quote on success. Mirrors the fixed tail of the attribute regex.
// Bounds-guarded byte walker (see `iter_attrs`): every `[i]` is `i < len`
// guarded; no slicing.
#[allow(clippy::indexing_slicing)]
fn match_eq_quote(bytes: &[u8], at: usize) -> Option<usize> {
  let len = bytes.len();
  let mut i = at;
...
/// The three ExifTool value-qualifier properties that `ParseXMPElement`
/// SUPPRESSES on the read path (XMP.pm:4202 `/^et:(desc|prt|val)$/`). Distinct
/// from [`is_ignore_et_prop`] (the `%ignoreEtProp` tag-NAME suppression):
/// these are dropped from EXTRACTION (with a `--$count` sibling-count
/// adjustment), so the surviving sibling value wins.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum EtQual {
  /// `et:desc` — ALWAYS suppressed (the `$1 eq 'desc'` arm of XMP.pm:4202).
  Desc,
  /// `et:prt` — suppressed only when not the first sibling (`$count` truthy).
  Prt,
  /// `et:val` — suppressed only when not the first sibling (`$count` truthy).
  Val,
}

/// Match the `/^et:(desc|prt|val)$/` qualifier regex (XMP.pm:4202) — the FULL
/// property name (the `prop_list` entry, already namespace-translated to the
/// `et` prefix). `None` for any other property.
fn et_qual_suffix(full_name: &str) -> Option<EtQual> {
  match full_name {
    "et:desc" => Some(EtQual::Desc),
    "et:prt" => Some(EtQual::Prt),
    "et:val" => Some(EtQual::Val),
    _ => None,
  }
}
...
/// Strip `<!-- … -->` comments (XMP.pm:4180, 4183 — `s/<!--.*?-->//g`).
///
/// CRITICAL: the Perl regex has NO `/s` flag, so `.` does NOT match a newline.
/// A comment span is therefore removed only when the **first** `-->` after the
/// `<!--` is reached by `.*?` WITHOUT crossing an `\n` (LF, `0x0A`) — i.e. the
/// minimal body contains no LF. If an LF appears before that first `-->`, the
/// match fails at this `<!--` position and Perl's `s///g` resumes one byte on
/// (so a later single-line `<!-- … -->` nested inside CAN still be stripped).
/// MULTILINE comments are thus PRESERVED verbatim. Verified vs perl 13.59:
/// `<!--\nabc\n-->` stays, `<!--abc\n<!--def-->` → `<!--abc\n` (inner stripped).
// Bounds-guarded byte/str walker: every `[i]` / `[a..b]` here is proven
// in-range by an explicit `i < end` / `i + N <= len` check (with `&&`
// short-circuit) or a fixed-size array, and the walker's `end`-window
...
fn strip_comments(s: &str) -> String {
  ...
  while i < bytes.len() {
    if bytes[i..].starts_with(b"<!--")
      ...
      // Non-dotall: the minimal `.*?` body (bytes between `<!--` and the first
      // `-->`) must contain no LF, else this candidate fails (resume +1 byte).
      && !bytes[i + 4..i + 4 + end].contains(&b'\n')
    {
      i += 4 + end + 3;
      continue;
    }
...
///
/// Mirrors [`strip_comments`]'s non-dotall rule: a `<!-- … -->` is removed only
/// when the minimal body up to the first `-->` contains no LF; otherwise the
/// `<` is emitted and scanning resumes one byte on (so an inner single-line
/// comment can still match), leaving multiline comments verbatim.
fn strip_comments_unicode(s: &str) -> String {
  ...
  while let Some(idx) = rest.find("<!--") {
    ...
    match after.find("-->") {
      // Single-line comment body (no LF before the first `-->`): strip it.
      Some(close) if !after[..close].contains('\n') => {
        rest = &after[close + 3..];
      }
      // Multiline (LF before `-->`) or unterminated: this `<!--` does not start
      // a removable comment. Emit the `<` and resume one byte on so a nested
      // single-line `<!-- … -->` can still be stripped.
      _ => {
        out.push('<');
        rest = &rest[idx + 1..];
      }
...
fn unescape_value_with_cdata_bytes(val: &[u8]) -> Vec<u8> {
  ...
  // The CDATA-special path activates ONLY when at least one COMPLETE
  // `<![CDATA[ … ]]>` pair exists (Perl `if ($val =~ /<!\[CDATA\[(.*?)\]\]>/sg)`
  // — `.*?` is dotall but the `]]>` is mandatory). An opening marker with no
  // close is NOT special: the WHOLE value (literal `<![CDATA[` text included)
  // goes through `UnescapeXML`. Verified vs perl 13.59: `pre<![CDATA[a&amp;b`
  // → `pre<![CDATA[a&b` (marker kept, `&amp;`→`&`).
  if next_complete_cdata(val).is_none() {
    return unescape_xml_bytes(val);
  }
  let mut out = Vec::with_capacity(val.len());
  let mut rest = val;
  // Each iteration consumes the NEXT complete pair; the loop ends as soon as
  // no further complete pair exists (mirroring the `while` re-match in
  // XMP.pm:3662), after which the remaining tail is unescaped (XMP.pm:3667).
  while let Some((idx, body_len)) = next_complete_cdata(rest) {
    out.extend_from_slice(&unescape_xml_bytes(&rest[..idx]));
    let body_start = idx + CDATA_OPEN.len();
    out.extend_from_slice(&rest[body_start..body_start + body_len]);
    rest = &rest[body_start + body_len + CDATA_CLOSE.len()..];
  }
  out.extend_from_slice(&unescape_xml_bytes(rest));
  out
}

/// Locate the next COMPLETE `<![CDATA[ … ]]>` pair in `bytes`, returning
/// `(open_offset, body_len)` where `body_len` is the byte length of the CDATA
/// body between `<![CDATA[` and the first following `]]>`. Returns `None` when
/// no opening marker is followed by a closing `]]>` — i.e. there is no complete
/// section (an unclosed `<![CDATA[` does not count). Faithful to Perl's
/// `/<!\[CDATA\[(.*?)\]\]>/sg`: `(.*?)` is dotall and minimal, so the body runs
/// to the NEAREST `]]>` after the opening marker.
// Bounds-guarded byte walker: every `[a..b]` is proven in-range by the
// `find_subslice` offsets it composes; covered by the file-level `#![deny]`.
#[allow(clippy::indexing_slicing)]
fn next_complete_cdata(bytes: &[u8]) -> Option<(usize, usize)> {
  const CDATA_OPEN: &[u8] = b"<![CDATA[";
  const CDATA_CLOSE: &[u8] = b"]]>";
  let idx = find_subslice(bytes, CDATA_OPEN)?;
  let body_start = idx + CDATA_OPEN.len();
  let close = find_subslice(&bytes[body_start..], CDATA_CLOSE)?;
  Some((idx, close))
}
...
impl Walker<'_> {
  ...
  fn found_xmp_full(
    ...
    //
    // A field-level `ValueConv => DecodeBase64` (`xmpGImg:image` of the
    // `%sThumbnail`/`%sPageInfo` structs, XMP.pm:367-372/383). Because
    // `DecodeBase64` returns a Perl scalar REF, its result is ALWAYS binary
    // (ExifTool.pm:3534 stops conversion on a SCALAR ref), rendering as the
    // `(Binary data N bytes, …)` placeholder.
    let field_base64 = matches!(
      field.map(tables::Field::value_conv),
      Some(tables::ValueConv::DecodeBase64)
    );
    // **Codex R6/B:** the `rdf:datatype="base64"` decode and the field
    // `ValueConv => DecodeBase64` are NOT mutually exclusive — Perl runs the
    // datatype decode FIRST (XMP.pm:3645) and then `ConvertValue` STILL applies
    // the field `ValueConv` IF the value is now a plain scalar. So a
    // `<xmpGImg:image rdf:datatype="...base64">aGVsbG8=</…>` is DOUBLE-decoded:
    // datatype `DecodeBase64("aGVsbG8=")` → `"hello"` (5 bytes, text → deref'd),
    // then field `DecodeBase64("hello")` → its uuencode bytes (43 bytes,
    // binary). But if the datatype decode yields BINARY it stays a SCALAR ref
    // and ConvertValue breaks (ExifTool.pm:3534) — the field ValueConv is then
    // SKIPPED and the datatype bytes are emitted as-is.
    let decoded = base64.then(|| decode_base64(raw_val));
    let value = match (base64, field_base64) {
      // ---- datatype base64 + field DecodeBase64 (the double-decode) --------
      (true, true) => {
        let step1 = decoded.expect("`base64` ⇒ `decoded` is Some");
        if base64_is_binary(&step1) {
          // Step1 binary ⇒ stays a SCALAR ref ⇒ ConvertValue breaks ⇒ the
          // field `DecodeBase64` is NOT applied; emit the datatype bytes.
          XmpValue::Binary(step1)
        } else {
          // Step1 deref'd to a scalar string ⇒ ConvertValue applies the field
          // `DecodeBase64` to the un-escaped + UTF8-decoded Step1 text
          // (XMP.pm:3647 deref → 3655-3669 unescape → ConvertValue ValueConv).
          // The field decode ALWAYS yields binary (scalar ref).
          let step1_text = crate::convert::fix_utf8(&unescape_value_with_cdata_bytes(&step1));
          XmpValue::Binary(decode_base64(&step1_text))
        }
      }
      // ---- field DecodeBase64 only (no base64 datatype) --------------------
      // Decode the UN-ESCAPED value (the field `ValueConv` fires AFTER FoundXMP
      // un-escapes + UTF8-decodes); always binary — `aGVs&#x62;G8=` →
      // `aGVsbG8=` → `hello` (5 bytes, binary placeholder).
      (false, true) => XmpValue::Binary(decode_base64(&unescape_value_with_cdata(raw_val))),
      // ---- datatype base64 only (no field DecodeBase64) --------------------
      (true, false) => match decoded.expect("`base64` ⇒ `decoded` is Some") {
        ...
        bytes if base64_is_binary(&bytes) => XmpValue::Binary(bytes),
        ...
        bytes => XmpValue::Scalar(scalar_from_text(
          crate::convert::fix_utf8(&unescape_value_with_cdata_bytes(&bytes)),
          field,
          writable,
        )),
      },
      // ---- neither: the ordinary un-escaped value --------------------------
      (false, false) => XmpValue::Scalar(scalar_from_text(
...
fn apply_value_conversions(
  ...
  out = match value_conv {
    ...
    // `DecodeBase64` (the `xmpGImg:image` thumbnail field) is handled UPSTREAM
    // in `found_xmp_full` (it yields an [`XmpValue::Binary`], never a scalar),
    // so this arm is never reached on the scalar path; leave the value
    // untouched for exhaustiveness.
    tables::ValueConv::DecodeBase64 => out,
  };
...
/// `DecodeBase64` (XMP.pm:2981) — faithful port of the three-step Perl decode.
///
/// `Image::ExifTool::XMP::DecodeBase64` is ALSO the decoder behind `Vorbis.pm`
/// COVERART / METADATA_BLOCK_PICTURE and `PLIST.pm` `data`, which in this crate
/// route through [`crate::convert::base64_decode`] (a standard-base64 decode).
/// The two agree byte-for-byte on well-formed, padded base64 (the only kind
/// real COVERART/PLIST payloads carry) and diverge only on malformed/partial
/// input; aligning that helper onto this uuencode algorithm is a separate
/// follow-up (it predates this branch and is exercised only on clean fixtures).
///
/// This is NOT standard bit-buffer base64: ExifTool translates the data to the
/// uuencode alphabet and decodes it with Perl's `unpack('u', …)`, whose
/// length-byte + 4-char-group machinery produces different byte COUNTS than a
/// straight base64 decode on PARTIAL/odd input (the count is what surfaces in
/// the `(Binary data N bytes, …)` placeholder, so it must match bundled). For
/// well-formed, padded base64 the two agree byte-for-byte.
///
...
/// in the list, so `aGVs\x0bbG8=` → `aGVs`). `\t\n\r\f` and space ARE in the
/// list (Rust [`u8::is_ascii_whitespace`] = `\t\n\x0c\r ` — note it excludes
/// VT, matching the Perl list exactly).
///
/// Step 2 (XMP.pm:2990, `tr/A-Za-z0-9+\/= \t\n\r\f/ -_/d`): map each of the 64
/// base64 digits `A-Za-z0-9+/` to a uuencode byte `0x20 + sextet` (space..`_`),
/// and DELETE `=` plus all whitespace (the `/d` flag — the replacement set ` -_`
/// is exhausted after the 64 digits, so `= \t\n\r\f` have no image).
///
/// Step 3 (XMP.pm:2993-3009): split the translated bytes into 60-char chunks,
/// prepend the uuencode length byte (`0x20 + len*3/4`, integer division), and
/// decode each with Perl's `unpack('u', …)` (see [`uudecode_line`]),
/// concatenating. The trailing partial chunk uses its own recomputed length
/// byte. This faithfully reproduces the partial-chunk quirks: `aGVsb` (5 chars)
/// → 30 bytes, `aGVsbG8`/`aGVsbG8=` → 5 bytes (`hello`), `AAAAA` → 3 bytes.
///
...
fn decode_base64(s: &str) -> Vec<u8> {
  ...
  // Steps 1+2: translate the surviving prefix to the uuencode alphabet.
  let mut uu: Vec<u8> = Vec::with_capacity(s.len());
  for &c in s.as_bytes() {
    // Step 1: the allow-list is `[A-Za-z0-9+/= \t\n\r\f]`. `=` and whitespace
    // stay in-list (then get deleted by step 2's `/d`); the first byte outside
    // the list (incl. VT `\x0b`) truncates the remainder.
    if c == b'=' || c.is_ascii_whitespace() {
      continue; // step 2 `/d`: deleted
    }
    let Some(v) = sextet(c) else { break }; // step 1 truncate
    uu.push(0x20 + v); // step 2: base64 digit → uuencode byte
  }

  // Step 3: chunked `unpack('u', uuLen . chunk)`. Each `unpack` is fed the
  // length byte PREPENDED to the chunk (exactly as Perl builds `uuLen . chunk`),
  // so [`uudecode_line`] applies the outer-loop guards to the length byte too.
  const CHUNK: usize = 60;
  let mut out = Vec::with_capacity(uu.len() * 3 / 4);
  let mut line = Vec::with_capacity(CHUNK + 1);
  // `chunks(60)` reproduces the Perl `for` loop (whole 60-byte chunks, uuLen =
  // 0x20 + 60*3/4 = 0x4d 'M') PLUS its trailing `if ($i < $len)` partial chunk
  // (XMP.pm:3006, uuLen = 0x20 + rem*3/4): a sub-60 final chunk gets its own
  // recomputed length byte, while a final chunk that fills 60 uses 'M' (an
  // exact multiple of 60 leaves no Perl remainder — same as here).
  for chunk in uu.chunks(CHUNK) {
    line.clear();
    // `pack('c', 32 + len*3/4)`: Perl computes a float then truncates toward
    // zero; for len ≤ 60 this equals integer `0x20 + (len*3)/4` (≤ 0x4d).
    line.push(0x20u8.wrapping_add((chunk.len() as u32 * 3 / 4) as u8));
    line.extend_from_slice(chunk);
    uudecode_line(&line, &mut out);
  }
  out
}

/// Perl `unpack('u', $line)` over one already-translated uuencode line —
/// the inner decode of [`decode_base64`] (`pp_pack.c` `case 'u'`).
///
/// `line` is the length byte PREPENDED to the chunk (the caller's
/// `0x20 + len*3/4` followed by the translated payload), matching Perl's
/// `unpack('u', uuLen . chunk)`. Faithful to the C:
///
/// - outer loop: while a byte remains AND it is not a space AND it is a uuchar
///   (`[0x20, 0x60)` = space..`_`), take it as the line length byte
///   (`len = byte & 0x3f`), then decode 4-char groups → 3 bytes while `len > 0`,
///   emitting `min(len, 3)` bytes per group and `len -= 3` each iteration;
/// - chars past the line end (or non-uuchars) contribute a 0 sextet;
/// - after a line's groups, leftover uuchars are re-read as a NEW length byte
///   (this is what makes `aGVsb` → 30 bytes), but a leftover SPACE stops the
///   outer loop (so `AAAAA`, which translates to spaces, → 3 bytes not 30; and a
///   SPACE length byte — `0x20 + len*3/4` when `len*3/4 == 0`, i.e. a 1-char
///   partial chunk like `a` — stops immediately → 0 bytes).
///
/// There are no embedded newlines in our lines, so the C trailing
/// `'\n'`/checksum skip is a no-op and is omitted.
fn uudecode_line(line: &[u8], out: &mut Vec<u8>) {
  // `ISUUCHAR(ch)` = ch in `[' ', 'a')` = `0x20..=0x5f`.
  fn is_uuchar(b: u8) -> bool {
    (0x20..0x60).contains(&b)
  }
  // `PL_uudmap[ch] & 077` = `(ch - 0x20) & 0x3f` over the uuchar range.
  fn uudmap(b: u8) -> u8 {
    b.wrapping_sub(0x20) & 0x3f
  }

  // The C reads the length byte from the very front of the stream under the
  // same outer-loop guard (`*s != ' ' && ISUUCHAR(*s)`) used for re-reads.
  let mut pos = 0usize;
  loop {
    // Outer-loop guard: stop at end, at a space, or at a non-uuchar.
    let len_byte = match line.get(pos) {
      Some(&b) if b != 0x20 && is_uuchar(b) => {
        pos += 1;
        b
      }
      _ => break,
    };
    let mut len = i32::from(uudmap(len_byte));
    while len > 0 {
      // Read up to 4 chars; a missing/non-uuchar char contributes sextet 0.
      let mut group = [0u8; 4];
      for slot in &mut group {
        if let Some(&b) = line.get(pos)
          && is_uuchar(b)
        {
          *slot = uudmap(b);
          pos += 1;
      ...
      // `len > 0` here ⇒ `emit ∈ {1, 2, 3}` ≤ `hunk.len()`, so `.get(..emit)` is
      // always `Some`; the checked form keeps `clippy::indexing_slicing` happy.
      let emit = if len > 3 { 3 } else { len as usize };
      if let Some(bytes) = hunk.get(..emit) {
...
/// The `rdf:Description` literal (`$rdfDesc`, WriteXMP.pl:39).
const RDF_DESC: &str = "rdf:Description";

/// Match a nodeID `$id` as Perl would AT a fixed offset (WriteXMP.pl:442
/// interpolates `$id` UNESCAPED into the match regex, so `$id` is a PATTERN,
/// not a literal). The realistic nodeID alphabet is `[^ /]*` (WriteXMP.pl:438)
/// — i.e. an NCName-ish token — whose only regex-significant character is `.`
/// (an NCName may contain `.`, but never the `*+?()[]{}|\^$` operators), so
/// the only Perl-regex semantic that affects a REAL collision is `.` = "match
/// any one char". This matcher reproduces exactly that: each `.` in `$id`
/// consumes any single byte; every other byte matches itself. (A malformed id
/// carrying an actual regex operator is outside the real-input contract and is
/// matched literally — the port's documented faithfulness scope.) Returns the
/// byte offset in `bytes` just past the matched id, or `None` on a mismatch.
fn match_node_id_at(bytes: &[u8], start: usize, id: &str) -> Option<usize> {
  let mut pos = start;
  for &ib in id.as_bytes() {
    let pb = *bytes.get(pos)?;
    // `.` matches any one byte (Perl `.` excludes only newline; a path holds
    // none); any other id byte must match literally.
    if ib != b'.' && ib != pb {
      return None;
    }
    pos += 1;
...
/// Split a property path at one nodeID `$id`, faithful to the Perl regex
/// `^(.*?)/([^/]*) #$id((/.*)?)$` (WriteXMP.pl:442). `$id` is interpolated
/// UNESCAPED, so it is matched as a PATTERN (see [`match_node_id_at`] — `.`
/// is a wildcard); a dotted id like `a.b` therefore COLLIDES with a sibling
/// ` #acb` exactly as bundled, instead of only the literal ` #a.b`. The
/// non-greedy `(.*?)` picks the FIRST `/<seg> #<id-pattern>` whose `<seg>`
/// (the element bearing the nodeID) holds no `/` and whose remainder starts
/// with `/` or ends the string. Returns `($pre, $prop, $post)` — the prefix
/// before that `/`, the element name `$prop`, and the suffix `$post` (`""`
/// or `/…`). `None` when the regex would not match (e.g. the nodeID is in the
/// very first segment, with no preceding `/` — Perl's `defined $pre` guard /
/// "internal error").
fn split_at_node_id(prop_path: &str, id: &str) -> Option<(String, String, String)> {
  let bytes = prop_path.as_bytes();
  // Scan every ` #` occurrence left-to-right; the non-greedy `(.*?)` makes the
  // FIRST one whose `<id-pattern>` matches AND whose match constraints hold
  // win. (The pre-R4 literal `find(" #id")` only ever matched a LITERAL id,
  // so a dotted id split at the wrong sibling vs Perl — Codex R4-C.)
  let mut from = 0usize;
  while let Some(rel) = prop_path.get(from..).and_then(|s| s.find(" #")) {
    let hash_at = from + rel; // byte offset of the leading space of ` #`
    // Match `$id` as a pattern at the byte right after ` #`.
    if let Some(after) = match_node_id_at(bytes, hash_at + 2, id) {
      // `((/.*)?)$`: the char right after the matched id must be `/` or EOS.
      let post_ok = after == bytes.len() || bytes.get(after) == Some(&b'/');
      if post_ok {
        // `([^/]*)` — the element segment must contain no `/`, and there must
        // be a `/` immediately before it (`(.*?)/`).
        let seg = prop_path.get(..hash_at).unwrap_or("");
        if let Some(slash) = seg.rfind('/') {
          let pre = seg.get(..slash).unwrap_or("").to_string();
          let prop = seg.get(slash + 1..).unwrap_or("").to_string();
          if !prop.contains('/') {
            let post = prop_path.get(after..).unwrap_or("").to_string();
            return Some((pre, prop, post));
          }
        }
        // No preceding `/` for this occurrence — Perl's `(.*?)/` cannot match
        // here, so the overall match fails (no later occurrence can satisfy
        // the anchored regex either): give up, mirroring the undef `$pre`.
        return None;
      }
    }
    from = hash_at + 1;
...
/// `SaveBlankInfo` (WriteXMP.pl:433). The propList is joined to a `$propPath`;
/// every ` #<id>` in it (`@ids`, WriteXMP.pl:438) is split out. The element
/// BEARING the nodeID is kept and routed per RDF semantics (WriteXMP.pl:446):
/// for a SUBJECT node (`$post` empty) it joins the PREFIX
/// (`$pre = "$pre/$prop"`); for an OBJECT node (`$post` set) it joins the
/// SUFFIX (`$post = "/$prop$post"`); but when the element IS `rdf:Description`
/// it stays out of both (the `unless ($prop eq $rdfDesc)` guard). When no
/// nodeID is present this is a no-op.
fn save_blank_info(blank: &mut BlankInfo, prop_list: &[SmolStr], val: &str, _extra: Option<()>) {
  let prop_path: String = prop_list
    .iter()
    .map(SmolStr::as_str)
    .collect::<Vec<_>>()
    .join("/");
  // `@ids = ($propPath =~ m{ #([^ /]*)}g)` — every nodeID in the path.
  let ids: Vec<String> = collect_node_ids(&prop_path);
  for id in ids {
    let Some((mut pre, prop, mut post)) = split_at_node_id(&prop_path, &id) else {
      // Perl: `warn("internal error parsing nodeID's"), next` — skip this id.
      continue;
    };
    // "the element with the nodeID should be in the path prefix for subject
    // nodes and the path suffix for object nodes" (WriteXMP.pl:444).
    if prop != RDF_DESC {
      if !post.is_empty() {
        post = std::format!("/{prop}{post}");
      } else {
        pre = std::format!("{pre}/{prop}");
      }
    }
    let node = blank.prop.entry(id).or_default();
    node.pre.insert(pre, ());
    if !post.is_empty() || !val.is_empty() {
      // Save the value + full path for each unique suffix (WriteXMP.pl:454).
      node.post.insert(post, (val.to_string(), prop_path.clone()));
    }
  }
}

/// `@ids = ($propPath =~ m{ #([^ /]*)}g)` (WriteXMP.pl:438): every nodeID,
/// i.e. each ` #` run up to the next space or `/`.
fn collect_node_ids(prop_path: &str) -> Vec<String> {
  let mut ids = Vec::new();
  let bytes = prop_path.as_bytes();
  let mut i = 0usize;
  while i + 1 < bytes.len() {
    if bytes.get(i) == Some(&b' ') && bytes.get(i + 1) == Some(&b'#') {
      ...
      ids.push(prop_path.get(start..j).unwrap_or("").to_string());
      i = j;
    } else {
      i += 1;
    }
...
/// `ProcessBlankInfo` (WriteXMP.pl:465). For each node, every SUBJECT prefix
/// (the ones containing `/rdf:Description/` — object prefixes are excluded
/// by WriteXMP.pl:481 `next unless $pre =~ m{/$rdfDesc/}`) is concatenated
/// with every property suffix and emitted through `FoundXMP`. Any suffix
/// never paired with a qualifying prefix (`%unused`, WriteXMP.pl:494 —
/// malformed RDF "about nothing", e.g. f-spot output) is emitted under its
/// ORIGINAL full path instead.
fn process_blank_info(walker: &mut Walker<'_>, blank: &BlankInfo) {
  let desc_mid = std::format!("/{RDF_DESC}/");
  for node in blank.prop.values() {
    // Track which suffixes get used by a qualifying subject prefix.
    let mut unused: BTreeMap<&String, ()> = node.post.keys().map(|k| (k, ())).collect();
    for pre in node.pre.keys() {
      // "there will be no description for the object of a blank node."
      if !pre.contains(&desc_mid) {
        continue;
      }
      for (post, (val, _)) in &node.post {
        let combined = std::format!("{pre}{post}");
        let props: Vec<SmolStr> = combined.split('/').map(SmolStr::new).collect();
        walker.found_xmp(&props, val, None);
        unused.remove(post);
      }
    }
    // `%unused` — suffixes with no subject: emit under their full propPath
    // (the `$et->Warn('An XMP resource is about nothing')` is Verbose-gated
    // in ExifTool and not surfaced here; the FoundXMP emission is not).
    for post in unused.keys() {
      if let Some((val, prop_path)) = node.post.get(*post) {
        let props: Vec<SmolStr> = prop_path.split('/').map(SmolStr::new).collect();
        walker.found_xmp(&props, val, None);
      }
    }
  }
...
impl crate::emit::Taggable for XmpMeta<'_> {
  ...
  /// is always `false`. The decode/walk `$et->Warn` is NOT a tag; it rides the
  /// [`Diagnose`](crate::diagnostics::Diagnose) channel.
  fn tags(
    &self,
...
#[cfg(feature = "alloc")]
impl crate::diagnostics::Diagnose for XmpMeta<'_> {
  /// Yield XMP's DOCUMENT-level `$et->Warn` (XMP sets no `SET_GROUP1`, so its
  /// warnings surface as `ExifTool:Warning`). XMP records a single
  /// first-occurrence warning (`XMP is double UTF-encoded` XMP.pm:4491, or a
  /// decode/walk warning — faithful `FoundTag('Warning')` first-wins), so this
  /// yields at most one [`Diagnostic`](crate::diagnostics::Diagnostic).
  fn diagnostics(&self) -> std::vec::Vec<crate::diagnostics::Diagnostic> {
    match &self.warning {
      Some(w) => std::vec![crate::diagnostics::Diagnostic::warn(w.as_str())],
...
#[cfg(feature = "alloc")]
impl XmpMeta<'_> {
  /// The post-ValueConv (`-n`) scalar text of the FIRST tag matching
  /// (`group`, `name`) — the machine value the domain projection reads (e.g.
  /// GPS as signed decimal degrees `45.5` / `-122.508…`, not the `-j` DMS
  /// string). Only a scalar leaf contributes; a list/struct/binary yields
  /// `None`. A single-element list DESCENDS to its scalar leaf — an XMP `List`
  /// camera tag (e.g. `ISOSpeedRatings` → `ISO`, XMP.pm:2068-2072 `List =>
  /// 'Seq'`) serializes even one value as a 1-item `rdf:Seq` (`-j`/`-n` `[100]`,
  /// matching bundled ExifTool, which the JSON output preserves verbatim), but
  /// the normalized domain projection wants the scalar `100`; a multi-element
  /// list / struct / binary still yields `None`. Faithful to reading the `-n`
  /// view, exactly as the EXIF/ID3 projections read their raw values.
  fn domain_numeric(&self, group: &str, name: &str) -> Option<&str> {
    let tag = self
      .tags
      .iter()
      .find(|t| t.group.as_str() == group && t.name.as_str() == name)?;
    // Descend a single-element `rdf:Seq`/`Bag`/`Alt` to its scalar leaf; a
    // multi-element list has no single domain value.
    let mut value = &tag.value;
    while let XmpValue::List(items) = value {
      if items.len() != 1 {
        return None;
      }
      value = items.first()?;
    }
    value.scalar_ref().map(XmpScalar::numeric)
  }

  /// The first (`group`, `name`) scalar's `-n` text as an owned, non-empty
  /// `String` (the camera/lens identity domain wants a clean string).
  fn domain_text(&self, group: &str, name: &str) -> Option<std::string::String> {
    self
      .domain_numeric(group, name)
      .filter(|s| !s.is_empty())
      .map(std::borrow::ToOwned::to_owned)
  }

  /// The first (`group`, `name`) scalar's `-n` text parsed as `f64` (a XMP
  /// numeric ValueConv value — e.g. `FNumber` `2.8`, `GPSLatitude` `45.5`).
  fn domain_f64(&self, group: &str, name: &str) -> Option<f64> {
    self.domain_numeric(group, name)?.parse::<f64>().ok()
  }

  /// The first (`group`, `name`) scalar's `-n` text parsed as `u32` (used for
  /// `ISO`, which XMP carries as the bare integer after the `ISOSpeedRatings`
  /// → `ISO` rename).
  fn domain_u32(&self, group: &str, name: &str) -> Option<u32> {
    self.domain_numeric(group, name)?.parse::<u32>().ok()
  }

  /// XMP-exif `GPSAltitude` is the UNSIGNED magnitude; `GPSAltitudeRef`
  /// (`0` above / `1` below sea level, XMP.pm:2329-2348) carries the sign.
  /// Mirrors the EXIF projector (`project.rs` `gps_altitude`): altitude
  /// absent ⇒ `None`; ref absent or `0` ⇒ positive; only ref `1` negates.
  fn gps_altitude(&self) -> Option<f64> {
    let alt = self.domain_f64("XMP-exif", "GPSAltitude")?;
    let below = self
      .domain_numeric("XMP-exif", "GPSAltitudeRef")
...
#[cfg(feature = "alloc")]
impl crate::metadata::Project for XmpMeta<'_> {
  /// Project an XMP sidecar onto the normalized cross-format domain. XMP
  /// mirrors the EXIF/TIFF facts a camera writes into the sidecar — the
  /// values are read from the post-ValueConv (`-n`) form so they are already
  /// machine-ready (GPS in signed decimal degrees, rationals as decimals):
  ///
  /// | domain field | XMP source (family-1 group : name) |
  /// |---|---|
  /// | `camera.make` | `XMP-tiff:Make` |
  /// | `camera.model` | `XMP-tiff:Model` |
  /// | `camera.software` | `XMP-xmp:CreatorTool` |
  /// | `camera.serial` | `XMP-aux:SerialNumber` |
  /// | `lens.model` | `XMP-aux:Lens` |
  /// | `lens.focal_length_mm` | `XMP-exif:FocalLength` |
  /// | `lens.aperture` | `XMP-exif:FNumber` |
  /// | `gps.{latitude,longitude}` | `XMP-exif:GPSLatitude` / `GPSLongitude` |
  /// | `gps.altitude_m` | `XMP-exif:GPSAltitude` |
  /// | `gps.timestamp` | `XMP-exif:GPSDateTime` (the `GPSTimeStamp` rename) |
  /// | `capture.exposure_time_s` | `XMP-exif:ExposureTime` |
  /// | `capture.iso` | `XMP-exif:ISO` (the `ISOSpeedRatings` rename) |
  /// | `capture.f_number` | `XMP-exif:FNumber` |
  ///
  /// The [`MediaInfo`](crate::metadata::MediaInfo) container domain stays
  /// empty — an `.xmp` sidecar carries no duration / track structure. Each
  /// domain is set only when non-empty, so a sidecar carrying none of these
  /// projects to the empty aggregate.
  fn project(&self) -> crate::metadata::MediaMetadata {
    use crate::metadata::{CameraInfo, CaptureSettings, GpsLocation, LensInfo, MediaMetadata};
    let mut out = MediaMetadata::new();

    // ---- CameraInfo (XMP-tiff Make/Model, XMP-xmp CreatorTool, XMP-aux serial)
    let mut camera = CameraInfo::new();
    camera
      .update_make(self.domain_text("XMP-tiff", "Make"))
      .update_model(self.domain_text("XMP-tiff", "Model"))
      .update_software(self.domain_text("XMP-xmp", "CreatorTool"))
      .update_serial(self.domain_text("XMP-aux", "SerialNumber"));
    if !camera.is_empty() {
      out.set_camera(camera);
    }

    // ---- LensInfo (XMP-aux Lens, XMP-exif FocalLength/FNumber) ----------
    let mut lens = LensInfo::new();
    lens
      .update_model(self.domain_text("XMP-aux", "Lens"))
      .update_focal_length_mm(self.domain_f64("XMP-exif", "FocalLength"))
      .update_aperture(self.domain_f64("XMP-exif", "FNumber"));
    if !lens.is_empty() {
      out.set_lens(lens);
    }

    // ---- GpsLocation (XMP-exif GPS* — already signed decimal in `-n`) ---
    let mut gps = GpsLocation::new();
    gps
      .update_latitude(self.domain_f64("XMP-exif", "GPSLatitude"))
      .update_longitude(self.domain_f64("XMP-exif", "GPSLongitude"))
      .update_altitude_m(self.gps_altitude())
      // `GPSTimeStamp` is renamed to `GPSDateTime` in the XMP exif table.
      .update_timestamp(self.domain_text("XMP-exif", "GPSDateTime"));
    if !gps.is_empty() {
      out.set_gps(gps);
    }

    // ---- CaptureSettings (XMP-exif ExposureTime/ISO/FNumber) ------------
    let mut capture = CaptureSettings::new();
    capture
      .update_exposure_time_s(self.domain_f64("XMP-exif", "ExposureTime"))
      .update_iso(self.domain_u32("XMP-exif", "ISO"))
      .update_f_number(self.domain_f64("XMP-exif", "FNumber"));
    if !capture.is_empty() {
...
#[cfg(test)]
#[allow(clippy::indexing_slicing)] // test-only: fixed-shape literals, panic == test failure
mod tests {
  ...
  // ----- Codex R2: unanchored attribute-scan recovery ---------------------
  /// `iter_attrs` mirrors the COMMON-branch (`length < 2000`) attribute regex
  /// `/(\S+?)\s*=\s*(['"])/g` (XMP.pm:3887), which is UNANCHORED + `/g`: a junk
  /// token with no following `=quote` is SKIPPED, not fatal. Each case below is
  /// byte-for-byte the output of running that exact Perl regex loop (XMP.pm:
  /// 3884-3900) against the input.
  #[test]
  fn iter_attrs_skips_malformed_tokens_like_perl() {
    use super::iter_attrs;
    let pair = |k: &str, v: &str| (k.to_string(), v.to_string());

    // The finding's case: junk between `xmlns:dc` and the shorthand property.
    assert_eq!(
      iter_attrs(
        r#"rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/" junk dc:title="Lost""#
      ),
      vec![
        pair("rdf:about", ""),
        pair("xmlns:dc", "http://purl.org/dc/elements/1.1/"),
        pair("dc:title", "Lost"),
      ],
    );
    // Junk in the MIDDLE / TRAILING / LEADING — scan must not terminate.
    assert_eq!(
      iter_attrs(r#"a="1" junk b='2'"#),
      vec![pair("a", "1"), pair("b", "2")],
    );
    assert_eq!(iter_attrs(r#"junk a="1""#), vec![pair("a", "1")]);
    assert_eq!(iter_attrs(r#"a="1" junk"#), vec![pair("a", "1")]);
    // `a=b` (no quote after `=`) is skipped entirely; the engine resumes at
    // the next `name = quote` (`c="2"`).
    assert_eq!(iter_attrs(r#"a=b c="2""#), vec![pair("c", "2")]);
    // Spaces around `=` are tolerated by `\s*=\s*`.
    assert_eq!(iter_attrs(r#"a = "1""#), vec![pair("a", "1")]);
    // A leading bare `=` has no `\S+?` name before it ⇒ no match there.
    assert_eq!(iter_attrs(r#"=  "noval" b="2""#), vec![pair("b", "2")],);
    // `\S+?` is non-space (it may contain a quote): the name is `a"`.
    assert_eq!(iter_attrs(r#"a"="1""#), vec![pair("a\"", "1")]);
    // Embedded `=` with no quote after any of them ⇒ whole token skipped.
    assert_eq!(iter_attrs(r#"foo=bar=baz x="9""#), vec![pair("x", "9")]);
    // An unterminated value ends the run (`last unless $attrs =~ /$quote/g`).
    assert_eq!(iter_attrs(r#"a="1" b="unterminated"#), vec![pair("a", "1")],);
    // The plain well-formed common case stays identical.
    assert_eq!(
      iter_attrs(r#"x:foo="bar" y:baz='qux'"#),
      vec![pair("x:foo", "bar"), pair("y:baz", "qux")],
    );
  }
  ...
  fn decode_base64_matches_perl_decodebase64() {
    ...
    // ----- Codex R6/A: PARTIAL-chunk uuencode math (NOT standard base64) ---
    // ExifTool decodes via `unpack('u', uuLen . chunk)`, whose length-byte +
    // 4-char-group machinery diverges from a straight base64 decode on odd
    // input. All counts oracle-verified vs `perl exiftool` 13.59 DecodeBase64.
    //
    // 5 surviving chars (`length % 4 == 1`): the first 4 chars decode to 3
    // real bytes, then the leftover char is re-read as a NEW (large) length
    // byte ⇒ 30 bytes total — NOT the 3-byte standard-base64 prefix `hel`.
    assert_eq!(decode_base64("aGVsb").len(), 30);
    assert_eq!(&decode_base64("aGVsb")[..3], b"hel");
    assert!(decode_base64("aGVsb")[3..].iter().all(|&b| b == 0));
    // Aligned partial groups (2/3/4 chars) match standard base64 byte counts.
    assert_eq!(decode_base64("aGVs"), b"hel"); // 4 chars → 3 bytes
    assert_eq!(decode_base64("aGV"), b"he"); //  3 chars → 2 bytes
    assert_eq!(decode_base64("aG"), b"h"); //    2 chars → 1 byte
    // A single surviving char ⇒ partial length byte is a SPACE (0x20 + 0),
    // which the `unpack('u')` outer-loop guard treats as end-of-data ⇒ EMPTY
    // (not 1 byte). `AAAAA` translates to spaces, so its leftover space also
    // stops the re-read ⇒ 3 bytes, NOT 30.
    assert_eq!(decode_base64("a"), Vec::<u8>::new());
    assert_eq!(decode_base64("AAAA"), vec![0, 0, 0]);
    assert_eq!(decode_base64("AAAAA"), vec![0, 0, 0]);
    // `length % 4 == 1` at 9 chars ⇒ 31 bytes (6 real `abcdef` from the first
    // 8 chars, then the leftover char is re-read as a length byte).
    assert_eq!(decode_base64("YWJjZGVmZ").len(), 31);
    assert_eq!(&decode_base64("YWJjZGVmZ")[..6], b"abcdef");
  }
  ...
  // ----- Codex R1/F1: non-dotall comment strip (XMP.pm:4180,4183) ---------
  // `s/<!--.*?-->//g` has NO `/s`, so `.` never matches LF ⇒ a comment is
  // removed only when the minimal body up to the FIRST `-->` has no LF;
  // multiline comments are PRESERVED. Verified vs perl 13.59.
  #[test]
  fn strip_comments_is_non_dotall() {
    use super::{strip_comments, strip_comments_and_trim, strip_comments_unicode};
    // Single-line comment: stripped.
    assert_eq!(strip_comments("<!--abc-->"), "");
    assert_eq!(strip_comments("x<!--abc-->y"), "xy");
    // Multiline comment: PRESERVED verbatim (LF before the first `-->`).
    assert_eq!(strip_comments("<!--\nabc\n-->"), "<!--\nabc\n-->");
    assert_eq!(strip_comments("x<!--\nabc\n-->y"), "x<!--\nabc\n-->y");
    // LF before the first `-->`, but an inner single-line comment follows:
    // Perl resumes one byte on and strips the inner one (→ `<!--abc\n`).
    assert_eq!(strip_comments("<!--abc\n<!--def-->"), "<!--abc\n");
    // First `-->` is after an LF ⇒ the whole candidate fails even though a
    // later `-->` exists on the same physical structure (→ unchanged).
    assert_eq!(strip_comments("<!--a\nb-->c-->"), "<!--a\nb-->c-->");
    // Mixed: single-line stripped, multiline kept (XMP.pm leftmost-resume).
    assert_eq!(
      strip_comments("a<!-- one --><!--\ntwo\n--><!-- three -->b"),
      "a<!--\ntwo\n-->b"
    );
    // Unterminated comment: kept verbatim.
    assert_eq!(strip_comments("pre<!--no close"), "pre<!--no close");
    // rdf:Description path also trims surrounding whitespace AFTER stripping.
    assert_eq!(strip_comments_and_trim("  <!--c-->  value  "), "value");
    assert_eq!(strip_comments_and_trim("  <!--\nc\n-->  "), "<!--\nc\n-->");
    // The non-ASCII (Unicode) path obeys the same non-dotall rule.
    assert_eq!(strip_comments_unicode("é<!--abc-->é"), "éé");
    assert_eq!(
      strip_comments_unicode("é<!--\nabc\n-->é"),
      "é<!--\nabc\n-->é"
    );
    assert_eq!(strip_comments_unicode("é<!--a\n<!--b-->é"), "é<!--a\né");
    // ASCII and Unicode paths agree on shared inputs.
    for s in [
      "<!--abc-->",
      "<!--\nabc\n-->",
      "<!--abc\n<!--def-->",
  ...
  // ----- Codex R1/F2: CDATA-special path requires a COMPLETE pair ---------
  // Perl `if ($val =~ /<!\[CDATA\[(.*?)\]\]>/sg)` enters the CDATA branch ONLY
  // when a complete `<![CDATA[ … ]]>` exists; an unclosed opening marker is NOT
  // special and the WHOLE value (literal marker included) is UnescapeXML'd.
  // Verified vs perl 13.59.
  #[test]
  fn cdata_unescape_requires_complete_pair() {
    use super::unescape_value_with_cdata_bytes as u;
    // Unclosed marker ⇒ whole value unescaped, literal `<![CDATA[` kept.
    assert_eq!(u(b"pre<![CDATA[a&amp;b"), b"pre<![CDATA[a&b");
    assert_eq!(u(b"a&amp;b<![CDATA[c&amp;d"), b"a&b<![CDATA[c&d");
    // Complete pair ⇒ body verbatim, surrounding text unescaped.
    assert_eq!(u(b"pre<![CDATA[a&amp;b]]>post&amp;x"), b"prea&amp;bpost&x");
    assert_eq!(u(b"<![CDATA[raw&amp;]]>"), b"raw&amp;");
    // Complete pair followed by an unclosed marker ⇒ first body verbatim, the
    // unclosed tail unescaped (literal marker kept).
    assert_eq!(u(b"<![CDATA[a]]>mid<![CDATA[b"), b"amid<![CDATA[b");
    // Unclosed marker BEFORE a complete pair: `.*?` (dotall) reaches the first
    // `]]>`, so the body spans the inner opener verbatim; the tail unescapes.
    assert_eq!(
      u(b"pre<![CDATA[no close <![CDATA[real]]>tail&amp;x"),
      b"preno close <![CDATA[realtail&x"
    );
    // No CDATA at all ⇒ plain whole-value unescape.
    assert_eq!(u(b"no cdata &amp; here"), b"no cdata & here");
  }
  ...
  // Codex R4-C: WriteXMP.pl:442 interpolates `$id` UNESCAPED, so `.` is a
  // regex wildcard in the nodeID split. These pin the Perl-regex semantics
  // (and that the R3-B literal-id behaviour still holds for dot-free ids).
  #[test]
  fn split_at_node_id_unescaped_regex_semantics() {
    use super::split_at_node_id;
    // Dot-free id: literal match, unchanged from R3-B.
    assert_eq!(
      split_at_node_id("rdf:Description/exif:Flash #n1", "n1"),
      Some(("rdf:Description".into(), "exif:Flash".into(), String::new()))
    );
    // A SUBJECT path with the id at the end of a `/`-delimited segment.
    assert_eq!(
      split_at_node_id("a/b/c:Tag #id1/d:Sub", "id1"),
      Some(("a/b".into(), "c:Tag".into(), "/d:Sub".into()))
    );
    // Dotted-id COLLISION: with ` #acb` BEFORE ` #a.b`, the unescaped `a.b`
    // pattern (`.`=any char) matches the EARLIER ` #acb` (Perl), splitting at
    // `seg1` — NOT the literal ` #a.b` at `seg2`. The pre-R4 literal needle
    // would have returned `("p/x #acb", "y", "")`.
    assert_eq!(
      split_at_node_id("p/x #acb/y #a.b", "a.b"),
      Some(("p".into(), "x".into(), "/y #a.b".into()))
    );
    // The non-dotted sibling id still splits at its own literal occurrence.
    assert_eq!(
      split_at_node_id("p/x #acb/y #a.b", "acb"),
      Some(("p".into(), "x".into(), "/y #a.b".into()))
    );
  }

  // Codex R4-C integration: `save_blank_info` must route a DOTTED colliding
  // nodeID the way Perl does (`.` = wildcard). With a path carrying ` #acb`
  // BEFORE ` #a.b`, the `a.b` split lands on the EARLIER `a:X #acb` segment
  // (so its `Pre` is just `rdf:Description`), NOT the literal `b:Y #a.b`
  // (which the pre-R4 literal needle picked, yielding the longer
  // `rdf:Description/a:X #acb/b:Y` prefix).
  #[test]
  fn save_blank_info_dotted_nodeid_collision_matches_perl() {
    use super::{BlankInfo, SmolStr, save_blank_info};
    let prop_list: Vec<SmolStr> = ["rdf:Description", "a:X #acb", "b:Y #a.b"]
      .iter()
      .map(|s| SmolStr::new(*s))
      .collect();
    let mut blank = BlankInfo::default();
    save_blank_info(&mut blank, &prop_list, "v", None);
    let dotted = blank.prop.get("a.b").expect("a.b node recorded");
    let pres: Vec<&str> = dotted.pre.keys().map(String::as_str).collect();
    // Perl/wildcard: the `.` matches `c`, so `a.b` splits at `a:X #acb` →
    // SUBJECT-routed `Pre = rdf:Description` (the literal needle would have
    // produced `rdf:Description/a:X #acb/b:Y`).
    assert_eq!(pres, vec!["rdf:Description"]);
  }

  #[test]
  fn match_node_id_at_dot_is_wildcard() {
    use super::match_node_id_at;
    // `.` consumes any one byte; literals must match.
    assert_eq!(match_node_id_at(b"acb", 0, "a.b"), Some(3));
    assert_eq!(match_node_id_at(b"axb", 0, "a.b"), Some(3));
    assert_eq!(match_node_id_at(b"a.b", 0, "a.b"), Some(3)); // `.` also matches `.`
    assert_eq!(match_node_id_at(b"axc", 0, "a.b"), None); // trailing `b` mismatch
    assert_eq!(match_node_id_at(b"ab", 0, "a.b"), None); // ran out of bytes
    // Offset + trailing context preserved (returns the end offset).
    assert_eq!(match_node_id_at(b"##acb/z", 2, "a.b"), Some(5));
  }

  // Codex R4-B: `et:desc`/`et:prt`/`et:val` value-qualifier suppression
  // (XMP.pm:4202) — `et:desc` always ignored; `et:prt`/`et:val` ignored only
  // when not the first sibling. So a `<foo:Tag>` with desc/prt/val children
  // emits the `et:prt` value.
  #[test]
  fn et_qualifier_suppression_emits_prt() {
    use super::{EtQual, et_qual_suffix};
    assert_eq!(et_qual_suffix("et:desc"), Some(EtQual::Desc));
    assert_eq!(et_qual_suffix("et:prt"), Some(EtQual::Prt));
    assert_eq!(et_qual_suffix("et:val"), Some(EtQual::Val));
    assert_eq!(et_qual_suffix("et:id"), None);
    assert_eq!(et_qual_suffix("dc:title"), None);

    // End-to-end: the bundled `XMP-foo:Tag=Print` result.
...
<?xpacket end="r"?>"#;
    let meta = super::parse_borrowed(xmp).expect("parses");
    let tag = meta
      .tags_slice()
      .iter()
      .find(|t| t.name() == "Tag")
      .expect("foo:Tag present");
    let scalar = tag.value_ref().scalar_ref().expect("scalar value");
