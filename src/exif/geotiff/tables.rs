...
/// The shared `%epsg_units` EPSG table (24 entries) — referenced by multiple GeoKeys.
pub(crate) static EPSG_UNITS: &[(i64, &str)] = &[
  (9001, "Linear Meter"),
  (9002, "Linear Foot"),
...
/// The shared `%epsg_vertcs` EPSG table (40 entries) — referenced by multiple GeoKeys.
pub(crate) static EPSG_VERTCS: &[(i64, &str)] = &[
  (0, "Undefined"),
...
/// `GTModelType` (GeoKey 1024) PrintConv (4 entries).
static GTMODELTYPE: &[(i64, &str)] = &[
  (1, "Projected"),
  (2, "Geographic"),
  (3, "Geocentric"),
  (32767, "User Defined"),
...
/// `GTRasterType` (GeoKey 1025) PrintConv (3 entries).
static GTRASTERTYPE: &[(i64, &str)] = &[
  (1, "Pixel Is Area"),
  (2, "Pixel Is Point"),
  (32767, "User Defined"),
];

/// `GeographicType` (GeoKey 2048) PrintConv (176 entries).
static GEOGRAPHICTYPE: &[(i64, &str)] = &[
  (4001, "Airy 1830"),
...
/// `GeogGeodeticDatum` (GeoKey 2050) PrintConv (157 entries).
static GEOGGEODETICDATUM: &[(i64, &str)] = &[
  (6001, "Airy 1830"),
...
/// `GeogPrimeMeridian` (GeoKey 2051) PrintConv (12 entries).
static GEOGPRIMEMERIDIAN: &[(i64, &str)] = &[
  (8901, "Greenwich"),
  (8902, "Lisbon"),
  (8903, "Paris"),
  (8904, "Bogota"),
...
/// `GeogEllipsoid` (GeoKey 2056) PrintConv (36 entries).
static GEOGELLIPSOID: &[(i64, &str)] = &[
  (7001, "Airy 1830"),
...
/// `ProjectedCSType` (GeoKey 3072) PrintConv (993 entries).
static PROJECTEDCSTYPE: &[(i64, &str)] = &[
...
/// `Projection` (GeoKey 3074) PrintConv (428 entries).
static PROJECTION: &[(i64, &str)] = &[
...
/// `ProjCoordTrans` (GeoKey 3075) PrintConv (27 entries).
static PROJCOORDTRANS: &[(i64, &str)] = &[
...
/// `ChartFormat` (GeoKey 47001) PrintConv (11 entries).
static CHARTFORMAT: &[(i64, &str)] = &[
  (47500, "General"),
  (47501, "Coastal"),
  (47502, "Harbor"),
...
/// `ChartSoundingDatum` (GeoKey 47008) PrintConv (14 entries).
static CHARTSOUNDINGDATUM: &[(i64, &str)] = &[
...
/// One GeoKey: its id, tag name, and (optional) int->label PrintConv slice.
/// `print_conv` is `None` for the name-only keys (Citations + numeric values).
pub(crate) struct GeoKeyDef {
  /// The GeoKey id (the `%GeoTiff::Main` hash key).
  pub(crate) id: u16,
  /// The tag Name (`Name => '…'`).
  pub(crate) name: &'static str,
  /// The int->label PrintConv slice, or `None` for a name-only key.
  pub(crate) print_conv: Option<&'static [(i64, &'static str)]>,
}

/// Every `%GeoTiff::Main` GeoKey, SORTED by id for `binary_search_by_key`.
/// (`GeoTiffVersion` id 1 is the synthetic version key — emitted by the
/// processor, never decoded from a GeoKey entry, but kept here for the name.)
pub(crate) static GEO_KEYS: &[GeoKeyDef] = &[
  GeoKeyDef {
    id: 1,
    name: "GeoTiffVersion",
    print_conv: None,
  ...
  GeoKeyDef {
    id: 1024,
    name: "GTModelType",
    print_conv: Some(GTMODELTYPE),
  },
  GeoKeyDef {
    id: 1025,
    name: "GTRasterType",
    print_conv: Some(GTRASTERTYPE),
  },
  GeoKeyDef {
    id: 1026,
    name: "GTCitation",
  ...
  GeoKeyDef {
    id: 2048,
    name: "GeographicType",
    print_conv: Some(GEOGRAPHICTYPE),
  },
  GeoKeyDef {
    id: 2049,
    name: "GeogCitation",
  ...
  GeoKeyDef {
    ...
    print_conv: Some(GEOGGEODETICDATUM),
  },
  GeoKeyDef {
    id: 2051,
    name: "GeogPrimeMeridian",
    print_conv: Some(GEOGPRIMEMERIDIAN),
  },
  GeoKeyDef {
    id: 2052,
    name: "GeogLinearUnits",
    print_conv: Some(EPSG_UNITS),
  },
  GeoKeyDef {
    id: 2053,
  ...
  GeoKeyDef {
    ...
    print_conv: Some(PROJECTEDCSTYPE),
  },
  GeoKeyDef {
    id: 3073,
    name: "PCSCitation",
  ...
  GeoKeyDef {
    id: 3074,
    name: "Projection",
    print_conv: Some(PROJECTION),
  },
  GeoKeyDef {
    id: 3075,
    name: "ProjCoordTrans",
    print_conv: Some(PROJCOORDTRANS),
  },
  GeoKeyDef {
    id: 3076,
  ...
  GeoKeyDef {
    ...
    print_conv: Some(CHARTSOUNDINGDATUM),
  },
  GeoKeyDef {
    id: 47009,
