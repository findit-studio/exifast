...
//! Pentax lens-type lookup — `%pentaxLensTypes` (`Pentax.pm:75-422`), the
//! `PrintConv` for the `LensType` tag (`%Pentax::LensRec` position 0, reached
//! via `%Pentax::Main` `0x003f LensRec`).
//!
//! The Perl hash keys are two-number strings (`'series model'`, e.g. `'3 44'`);
//! the camera writes them as an `int8u[2]` pair, so the lookup key here is the
//! `(series, model)` byte pair. The decimal sub-keys (`'3 23.1'` etc) are
//! write-time disambiguations ExifTool cannot derive from the two bytes alone
//! (the camera value only ever matches the integer-pair key), so they are
//! intentionally excluded.
//!
//! 265 integer-pair rows, sorted by `(series, model)` for binary search.

#![deny(clippy::indexing_slicing)]

use smol_str::SmolStr;

/// `%pentaxLensTypes` integer-pair rows — `((series, model), name)`, sorted by
/// the `(series, model)` key (binary-search-ready).
pub const PENTAX_LENS_TYPES: &[((u8, u8), &str)] = &[
  ((0, 0), "M-42 or No Lens"),
  ((1, 0), "K or M Lens"),
  ((2, 0), "A Series Lens"),
  ((3, 0), "Sigma"),
...
/// Look up a `(series, model)` byte pair against `%pentaxLensTypes` (the
/// DIRECT-key path). Returns the resolved lens name, or `None` when absent.
#[must_use]
pub fn lookup_direct(series: u8, model: u8) -> Option<SmolStr> {
  match PENTAX_LENS_TYPES.binary_search_by_key(&(series, model), |&(k, _)| k) {
    Ok(i) => PENTAX_LENS_TYPES
      .get(i)
...
/// Look up a `(series, model)` pair, applying the `%pentaxLensTypes` `OTHER`
/// firmware-rewrite sub (`Pentax.pm:81-92`) when the direct key misses.
///
/// The `OTHER` sub rewrites the SERIES byte by firmware quirk, in order, and
/// returns the matched name suffixed with the ORIGINAL `"(series model)"`
/// string (faithful to the Perl `return "...($_[0])"`):
///
/// 1. `*istD` may report series 4 for series-7 lenses: rewrite `4 -> 7`,
///    suffix ` (orig)`.
/// 2. Bodies that don't recognize SDM lenses (older K10 firmware) may report
///    series 7 instead of 8: rewrite `7 -> 8`, suffix ` ? (orig)`.
/// 3. 645D FA/DFA inconsistency: rewrite `11 -> 13` OR `13 -> 11`, suffix
///    ` ? (orig)`.
///
/// Each rewrite fires only when the rewritten pair IS a known key; the first
/// match wins, exactly as the Perl short-circuits.
#[must_use]
pub fn lookup_with_other(series: u8, model: u8) -> Option<SmolStr> {
  if let Some(name) = lookup_direct(series, model) {
    return Some(name);
  }
  // Build "<name><sep><series> <model>)" — the Perl suffix carries the ORIGINAL
  // pair string `$_[0]` (pre-rewrite series), e.g. `... (4 19)`.
  let with_orig = |name: &str, sep: &str| -> SmolStr {
    SmolStr::from(std::format!("{name}{sep}{series} {model})"))
  };
  if series == 4
    && let Some(name) = lookup_direct(7, model)
  {
    return Some(with_orig(&name, " ("));
  }
  if series == 7
    && let Some(name) = lookup_direct(8, model)
  {
    return Some(with_orig(&name, " ? ("));
  }
  if series == 11
    && let Some(name) = lookup_direct(13, model)
  {
    return Some(with_orig(&name, " ? ("));
  }
  if series == 13
    && let Some(name) = lookup_direct(11, model)
  {
    return Some(with_orig(&name, " ? ("));
  }
  None
